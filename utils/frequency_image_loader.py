...
class ImageLoader:
    ...
    def __init__(self, data_path, fr=10,low_pass=True,channel=None, batch_size=1,
                 image_res=(1080, 1920), homography_res=(880, 1600),
                 shuffle=True, vertical_flips=True, horizontal_flips=True,
                 idx_subset=None, crop_to_homography=False,start_dis=0.2,end_dis=0.3):
        ...
        self.low_mask=self.img_low_mask()
        self.high_mask=self.img_high_mask()
    ...
    def load_batch(self, images):
        ...
        ims = torch.stack([im for im, _, _,_,_ in im_res_name], 0)
        imn = torch.stack([im for _, im, _,_,_ in im_res_name], 0)
        return (ims,imn,
                [dis for _, _,dis,_,_,in im_res_name],
                [res for _, _,_,res, _ in im_res_name],
                [name for _,_,_, _, name in im_res_name])

    def img_low_mask(self):
        mask_size=(self.image_res[0],self.image_res[1])
        mask=np.zeros(mask_size)
        length_tuple=(mask_size[0],mask_size[1])
        center_tuple=(mask_size[0]//2,mask_size[1]//2)
        
        for x in range(length_tuple[0]):
            for y  in range(length_tuple[1]):
                if(x-center_tuple[0])**2+(y-center_tuple[1])**2<self.fr**2:
                        mask[x,y]=1


        return mask

    def img_high_mask(self):
        mask_size=(self.image_res[0],self.image_res[1])
        mask=np.zeros(mask_size)
        length_tuple=(mask_size[0],mask_size[1])
        center_tuple=(mask_size[0]//2,mask_size[1]//2)
    ...
    def img_filter(self,img,low_pass,fr):
        img=(img-np.min(img))/(np.max(img)-np.min(img))*255

        
        img_ffted=np.fft.fft2(img)
        img_shift=np.fft.fftshift(img_ffted)
        
        
        temp=np.log(np.abs(img_shift[:,:]))
        temp=(temp-np.min(temp))/(np.max(temp)-np.min(temp))*255

        mask=self.low_mask if low_pass else self.high_mask

        masked_ffted=img_shift*mask
        unsift_img=np.fft.ifftshift(masked_ffted)
        ifft_img=np.fft.ifft2(unsift_img)
        
        res_img=ifft_img.real
    ...
    def load_image(self, filenum, *augmentation_states):
        im = cv2.imread(self.im_names[filenum])
        im=cv2.resize(im,(self.image_res[1],self.image_res[0]))
        ...
        ##ここで、imに対してフィルターをかけてしまう
        im=self.img_filter(im,self.low_pass,self.fr)
        ...
        distance=random.uniform(self.start_dis,self.end_dis)
        distance=round(distance,2)
        target_img=im
        complex_img=im*np.exp(1j*distance)
        real_img=np.real(complex_img)
        imag_img=np.imag(complex_img)

        input_img=np.concatenate((real_img,imag_img),axis=0)
        ...
        return (torch.from_numpy(input_img).float(),torch.from_numpy(im).float(),
                input_res,distance,
