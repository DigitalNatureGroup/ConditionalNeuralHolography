...
class ImageLoader:
    ...
    def load_batch(self, images):
        ...
        ims = torch.stack([im for im, _, _,_,_,_ in im_res_name], 0)
        imn = torch.stack([im for _, im, _,_,_,_ in im_res_name], 0)
        imm =torch.stack([im for _,_,im,_,_,_ in im_res_name],0)
        return (ims,imn,imm,
                [dis for _, _,_,dis,_,_,in im_res_name],
                [res for _,_, _,_,res, _ in im_res_name],
                [name for _,_,_,_, _, name in im_res_name])
    ...
    def load_image(self, filenum, *augmentation_states):
        ...
        return (torch.from_numpy(real_img).float(),torch.from_numpy(imag_img).float(),torch.from_numpy(im).float(),
                input_res,distance,
