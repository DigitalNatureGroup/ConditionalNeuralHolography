...
class ImageLoader:
    ...
    def load_image(self, filenum, *augmentation_states):
        ...
        full_shape=im.shape
        full_matrix=np.full(im.shape,distance)
        # input_img=np.concatenate((im,full_matrix),axis=0)
        ...
        return (torch.from_numpy(full_matrix).float(),torch.from_numpy(im).float(),
                input_res,distance,
