...
run_id = f"{status_name}_{model_type_name}_{distance_to_image_name}" if gen_type==0 else gen_string
...
with torch.no_grad():
    if(gen_type==2 or gen_type==3):
        num_iters_array=[1,11,21,31,41,51,61,71,81,91,101]
    else :
        num_iters_array=[1]
    for num_iters in num_iters_array:
    
        for k, target in enumerate(val_loader):
            ...
            if gen_type == 2:
                ...
                phase_only_algorithm = SGD(distance, wavelength, feature_size, opt.num_iters, roi_res, root_path,
                                "ASM",propagation_ASM, nn.MSELoss().to(device), 8e-3, 2e-3, s0, opt.citl, None, writer, device)
                start_time=time.perf_counter()
                _,slm_phase=phase_only_algorithm(target_amp)
                computing_time=time.perf_counter()-start_time
            #use GS
            elif gen_type == 3:
                phase_only_algorithm = GS(distance, wavelength, feature_size, opt.num_iters, root_path,
